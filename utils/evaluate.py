...
def compute_metrics(query_points, trajs_g, visibs_g, trajs_e, visibs_e, first=True, framewise=False):
    ...
    if first:
      ...
      if framewise:
        outputs = compute_tapvid_metrics_frame(query_points, gt_occluded, gt_tracks, pd_occluded, pd_tracks, 'first')
      else:
        outputs = compute_tapvid_metrics(query_points, gt_occluded, gt_tracks, pd_occluded, pd_tracks, 'first')
    else:
      xys = query_points[...,1:][:,:,[1, 0]] * s
      time_dim = query_points[...,0].astype(int)[..., np.newaxis]
      # prepending a column to be -> (B, N, 3)
      query_points = np.concatenate((time_dim, xys), axis=-1)
      if framewise:
        outputs = compute_tapvid_metrics_frame(query_points, gt_occluded, gt_tracks, pd_occluded, pd_tracks, 'strided')
      else:
        outputs = compute_tapvid_metrics(query_points, gt_occluded, gt_tracks, pd_occluded, pd_tracks, 'strided')

    # taking the mean across videos
    metrics = {}
    for key, value in outputs.items():
      #print(value.shape)
      metrics[key] = np.mean(value, axis=0) if framewise else value.mean().item()
...
def compute_tapvid_metrics_frame(
    query_points: np.ndarray,
    gt_occluded: np.ndarray,
    gt_tracks: np.ndarray,
    pred_occluded: np.ndarray,
  ...
  B, N, T = gt_occluded.shape
  # Occlusion accuracy is simply how often the predicted occlusion equals the
  # ground truth.
  occ_acc = np.sum(np.equal(pred_occluded, gt_occluded), axis=1) / N
  ...
  l2_dists = np.linalg.norm(pred_tracks - gt_tracks, axis=-1)
  thrs = 50 # threshold pixels for tracking failure
  dist_ok = 1 - (l2_dists > thrs)
  survival = np.ma.cumprod(dist_ok, axis=2)
  survival = np.mean(survival, axis=1) * 100
  #survival = np.mean(np.mean(survival, axis=2), axis=1) * 100
  metrics['survival'] = survival
  # mte = np.ma.mean(np.ma.median(l2_dists, axis=2), axis=1)
  mte = np.ma.median(l2_dists, axis=1)
  metrics['median_traj_error'] = mte
  ...
  for thresh in [1, 2, 4, 8, 16]:
    ...
    # within the threshold among points that are visible in the ground truth,
    # ignoring whether they're predicted to be visible.
    count_correct = np.sum(is_correct,axis=1)
    count_visible_points = np.sum(visible, axis=1)
    frac_correct = count_correct / count_visible_points
    metrics['pts_within_' + str(thresh)] = frac_correct
    all_frac_within.append(frac_correct)
    true_positives = np.sum(is_correct & pred_visible, axis=1)
    ...
    # False positives are simply points that are predicted to be visible,
    # but the ground truth is not visible or too far from the prediction.
    gt_positives = np.sum(visible, axis=1)
    false_positives = (~visible)
    false_positives = false_positives | ((~within_dist) & pred_visible)
