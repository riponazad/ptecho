...
from models.blocks import CMDTop, Conv2dSamePadding, ResNet_loco, ExtraConvs_loco

from utils.utils_ import (
    convert_grid_coordinates, map_coordinates_3d, is_same_res, bilinear,
    heatmaps_to_points, generate_default_resolutions, map_coordinates_2d
)
...
class LocoTrack(nn.Module):
  ...
  def __init__(
    ...
    self.cost_conv = Conv2dSamePadding(2, 1, 3, 1)
    self.occ_linear = nn.Linear(6, 2)

    if extra_convs:
      self.extra_convs = ExtraConvs_loco()
    else:
  ...
  def forward(
    ...
    # print(query_features.lowres[0].shape, query_features.hires[0].shape, query_features.highest[0].shape, query_features.resolutions[0])
